# Riferimento tecnico di Jarvis

Spostato da `CLAUDE.md` il 23/09/2026 (testo invariato). Da leggere **prima di modificare**
`index.html`, `scripts/` o il motore del consiglio; non serve a ogni sessione.
...
`prove/app.js` estrae lo `<script>` da `index.html`, lo esegue in Node con un finto DOM e una `fetch`
che legge da disco: **ogni funzione nuova aggiunge qui le sue verifiche**, e **ogni prova passa i
propri dati** invece di leggere i file veri che cambiano. Date con orologio finto e `TZ=Europe/Rome`.
