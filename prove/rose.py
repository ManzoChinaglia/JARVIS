...
with tempfile.TemporaryDirectory() as d:
    ...
    # scambio: Wesley (BURKINA FASO) per Dimarco (la sua squadra dai dati: nel file, che è
    # pubblico, nessun nome di un'altra squadra)
    squadra_dimarco = next(p[4] for p in base['p'] if p[1] == 'Dimarco')
    scambiato = copy.deepcopy(base['p'])
    for p in scambiato:
        if p[1] == 'Wesley': p[4] = squadra_dimarco
